-- The goal in this test is to find "being" as a match for "been", but both SQLean and C# phonetic code fail without wrapping results with distance methods
-- fp_JW_VS (fuzzy_phonetic, JaroWinkler, TRUE) comes close, but it also gives false matches for ban and bone
select Words, soundex(Words) as s, rsoundex(Words) as r, fuzzy_phonetic(Words) as p, fuzzy_caver(Words) as c, fuzzy_translit(Words) as t, Caverphone2(Words) as c2, Soundex2(Words) as s2
