...
#include "fuzzy.h"
#define is_ascii(x) isascii((unsigned int)x[0])
...
// Below are functions extracted from the spellfix SQLite extension
...
int sqlite_fuzzy_init(sqlite3* db, const sqlite3_api_routines* sqlite_api) {
    ...
    sqlite3_create_function(db, "JARO_WINKLER_DISTANCE", 2, flags, 0, fuzzy_jarowin, 0, 0);
