...
Example Usage:		(Using sqlite3pp::Table container)
	// Example #1
		...
		sqlite3pp::Table<sql_DB_sakila_table_actor> my_tbl;
		for (auto row : my_tbl)
			std::wcout << row << std::endl;

	// Example #2
		for (int i = 0; i < my_tbl.size(); ++i)
			std::wcout << my_tbl[i].get_actor_id() << std::endl;
...
*/
#ifndef SQL_DB_SAKILA_TABLE_ACTOR_H
#define SQL_DB_SAKILA_TABLE_ACTOR_H
#include <string>
#include "..\sqlite3pp_ez.h"

class sql_DB_sakila_table_actor: public sqlite3pp::sql_base
{
...
public:
	...
	// Constructors
	sql_DB_sakila_table_actor() {} // Default constructor
	template <class T> sql_DB_sakila_table_actor(const T &t) { setData(t); } // Allows data input from different (or same) tables/views having the same data types and column names
	...
	Example Usage:		(Using sqlite3pp::TableOStream container)
			TableOStream<sql_DB_sakila_table_actor> tbl(DbFileNameArg("myDatabase.db"));
	...
	*/
	...
	friend std::ostream& operator<<(std::ostream& os, const sql_DB_sakila_table_actor& t);
	friend std::wostream& operator<<(std::wostream& os, const sql_DB_sakila_table_actor& t);
	static StrType Delimiter() { return "," ; }
	////////////////////////////////////////////////////////////////////////////////////////////
};
// sqlite3pp::TableOStream container interface.
inline std::ostream& operator<<(std::ostream& os, const sql_DB_sakila_table_actor& t) { sqlite3pp::ostream_a o(os, t.Delimiter()); return t.OStream(o).os; }
inline std::wostream& operator<<(std::wostream& os, const sql_DB_sakila_table_actor& t) { sqlite3pp::ostream_w o(os, t.Delimiter());  return t.OStream(o).os; }

#endif // !SQL_DB_SAKILA_TABLE_ACTOR_H
