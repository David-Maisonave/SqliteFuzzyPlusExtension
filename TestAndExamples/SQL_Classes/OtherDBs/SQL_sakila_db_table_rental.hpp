...
Example Usage:		(Using sqlite3pp::Table container)
	// Example #1
		...
		sqlite3pp::Table<SQL_sakila_db_table_rental> my_tbl;
		for (auto& row : my_tbl)
			std::cout << row << std::endl;

	// Example #2
		for (int i = 0; i < my_tbl.size(); ++i)
			std::cout << my_tbl[i].get_rental_id() << std::endl;
...
*/
#ifndef SQL_SAKILA_DB_TABLE_RENTAL_H
#define SQL_SAKILA_DB_TABLE_RENTAL_H
#include <string>
#include "..\..\sqlite3pp_ez\sqlite3pp_ez.h"

class SQL_sakila_db_table_rental: public sqlite3pp::sql_base
{
	// A member variable for each field in the table
protected:
	Int rental_id = 0;
	Bigint rental_date = 0;
	Int inventory_id = 0;
	Int customer_id = 0;
	Bigint return_date = 0;
	Smallint staff_id = 0;
...
public:
	...
	// Constructors
	SQL_sakila_db_table_rental() {} // Default constructor
	template <class T> SQL_sakila_db_table_rental(const T &t) { setData(t); } // Allows data input from different (or same) tables/views having the same data types and column names
	...
	Example Usage:		(Using sqlite3pp::TableOStream container)
			TableOStream<SQL_sakila_db_table_rental> tbl(DbFileNameArg("myDatabase.db"));
	...
	*/
	...
	friend std::ostream& operator<<(std::ostream& os, const SQL_sakila_db_table_rental& t);
	friend std::wostream& operator<<(std::wostream& os, const SQL_sakila_db_table_rental& t);
	static StrType Delimiter() { return "," ; }
	////////////////////////////////////////////////////////////////////////////////////////////
};
// sqlite3pp::TableOStream container interface.
inline std::ostream& operator<<(std::ostream& os, const SQL_sakila_db_table_rental& t) { sqlite3pp::ostream_a o(os, t.Delimiter()); return t.OStream(o).os; }
inline std::wostream& operator<<(std::wostream& os, const SQL_sakila_db_table_rental& t) { sqlite3pp::ostream_w o(os, t.Delimiter());  return t.OStream(o).os; }

#endif // !SQL_SAKILA_DB_TABLE_RENTAL_H
