...
Example Usage:		(Using sqlite3pp::Table container)
	// Example #1
		...
		sqlite3pp::Table<SQL_chinook_db_table_employees> my_tbl;
		for (auto& row : my_tbl)
			std::cout << row << std::endl;

	// Example #2
		for (int i = 0; i < my_tbl.size(); ++i)
			std::cout << my_tbl[i].get_EmployeeId() << std::endl;

	// Example #3
		for (auto r = my_tbl.begin(); r != my_tbl.end(); ++r)
			std::cout << r->get_Email() << std::endl;
...
*/
#ifndef SQL_CHINOOK_DB_TABLE_EMPLOYEES_H
#define SQL_CHINOOK_DB_TABLE_EMPLOYEES_H
#include <string>
#include "..\..\sqlite3pp_ez\sqlite3pp_ez.h"

class SQL_chinook_db_table_employees: public sqlite3pp::sql_base
{
...
public:
	...
	// Constructors
	SQL_chinook_db_table_employees() {} // Default constructor
	template <class T> SQL_chinook_db_table_employees(const T &t) { setData(t); } // Allows data input from different (or same) tables/views having the same data types and column names
	...
	Example Usage:		(Using sqlite3pp::TableOStream container)
			TableOStream<SQL_chinook_db_table_employees> tbl(DbFileNameArg("myDatabase.db"));
	...
	*/
	...
	friend std::ostream& operator<<(std::ostream& os, const SQL_chinook_db_table_employees& t);
	friend std::wostream& operator<<(std::wostream& os, const SQL_chinook_db_table_employees& t);
	static StrType Delimiter() { return "," ; }
	////////////////////////////////////////////////////////////////////////////////////////////
};
// sqlite3pp::TableOStream container interface.
inline std::ostream& operator<<(std::ostream& os, const SQL_chinook_db_table_employees& t) { sqlite3pp::ostream_a o(os, t.Delimiter()); return t.OStream(o).os; }
inline std::wostream& operator<<(std::wostream& os, const SQL_chinook_db_table_employees& t) { sqlite3pp::ostream_w o(os, t.Delimiter());  return t.OStream(o).os; }

#endif // !SQL_CHINOOK_DB_TABLE_EMPLOYEES_H
