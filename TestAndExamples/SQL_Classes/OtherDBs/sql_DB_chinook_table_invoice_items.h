...
Example Usage:		(Using sqlite3pp::Table container)
	// Example #1
		...
		sqlite3pp::Table<sql_DB_chinook_table_invoice_items> my_tbl;
		for (auto row : my_tbl)
			std::wcout << row << std::endl;

	// Example #2
		for (int i = 0; i < my_tbl.size(); ++i)
			std::wcout << my_tbl[i].get_InvoiceLineId() << std::endl;

	// Example #3
		for (auto r = my_tbl.begin(); r != my_tbl.end(); ++r)
			std::wcout << r->get_Quantity() << std::endl;
...
class sql_DB_chinook_table_invoice_items: public sqlite3pp::sql_base
{
...
public:
	...
	// Constructors
	sql_DB_chinook_table_invoice_items() {} // Default constructor
	template <class T> sql_DB_chinook_table_invoice_items(const T &t) { setData(t); } // Allows data input from different (or same) tables/views having the same data types and column names
	...
	Example Usage:		(Using sqlite3pp::TableOStream container)
			TableOStream<sql_DB_chinook_table_invoice_items> tbl(DbFileNameArg("myDatabase.db"));
	...
	*/
	...
	friend std::ostream& operator<<(std::ostream& os, const sql_DB_chinook_table_invoice_items& t);
	friend std::wostream& operator<<(std::wostream& os, const sql_DB_chinook_table_invoice_items& t);
