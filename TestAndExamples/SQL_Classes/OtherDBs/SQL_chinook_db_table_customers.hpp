...
Example Usage:		(Using sqlite3pp::Table container)
	// Example #1
		...
		sqlite3pp::Table<SQL_chinook_db_table_customers> my_tbl;
		for (auto& row : my_tbl)
			std::cout << row << std::endl;

	// Example #2
		for (int i = 0; i < my_tbl.size(); ++i)
			std::cout << my_tbl[i].get_CustomerId() << std::endl;

	// Example #3
		for (auto r = my_tbl.begin(); r != my_tbl.end(); ++r)
			std::cout << r->get_SupportRepId() << std::endl;
...
*/
#ifndef SQL_CHINOOK_DB_TABLE_CUSTOMERS_H
#define SQL_CHINOOK_DB_TABLE_CUSTOMERS_H
#include <string>
#include "..\..\sqlite3pp_ez\sqlite3pp_ez.h"

class SQL_chinook_db_table_customers: public sqlite3pp::sql_base
{
...
protected:
	...
	Nvarchar Fax;
	Nvarchar Email;
	Integer SupportRepId = 0;

public:
	using StrType = std::string;
	// Constructors
	SQL_chinook_db_table_customers() {} // Default constructor
	template <class T> SQL_chinook_db_table_customers(const T &t) { setData(t); } // Allows data input from different (or same) tables/views having the same data types and column names
	...
	StrType GetValues() const
	{
		...
		strtype +=  ",'" + sqlite3pp::to_string( Company) + "'";
		...
		strtype +=  ",'" + sqlite3pp::to_string( SupportRepId) + "'";
		return strtype;
	}
	template<class T> void getStreamData( T q ) { q.getter()  >> CustomerId >> FirstName >> LastName >> Company >> Address >> City >> State >> Country >> PostalCode >> Phone >> Fax >> Email >> SupportRepId;}
	template <class T> void setData(const T &t) // Used to transfer data from different tables/views having same data types and column names
	{
		...
		Fax = t.get_Fax();
		Email = t.get_Email();
		SupportRepId = t.get_SupportRepId();
	}
	...
	const Nvarchar& get_Company() const {return Company;}
	...
	const Integer& get_SupportRepId() const {return SupportRepId;}
	...
	void set_Company(const Nvarchar& data__) {Company = data__;}
	...
	void set_SupportRepId(const Integer& data__) {SupportRepId = data__;}

	////////////////////////////////////////////////////////////////////////////////////////////
	/* sqlite3pp::TableOStream container interface.
	Functions OStream(), operator<<(), and Delimiter() are required when using the sqlite3pp::TableOStream container.
	Example Usage:		(Using sqlite3pp::TableOStream container)
			TableOStream<SQL_chinook_db_table_customers> tbl(DbFileNameArg("myDatabase.db"));
	...
	*/
	...
	{
		t.os << CustomerId << t.d << t.str(FirstName) << t.d << t.str(LastName) << t.d << t.str(Company) << t.d << t.str(Address) << t.d << t.str(City) << t.d << t.str(State) << t.d << t.str(Country) << t.d << t.str(PostalCode) << t.d << t.str(Phone) << t.d << t.str(Fax) << t.d << t.str(Email) << t.d << SupportRepId;
		return t;
	}
	friend std::ostream& operator<<(std::ostream& os, const SQL_chinook_db_table_customers& t);
	friend std::wostream& operator<<(std::wostream& os, const SQL_chinook_db_table_customers& t);
	static StrType Delimiter() { return "," ; }
	////////////////////////////////////////////////////////////////////////////////////////////
};
// sqlite3pp::TableOStream container interface.
inline std::ostream& operator<<(std::ostream& os, const SQL_chinook_db_table_customers& t) { sqlite3pp::ostream_a o(os, t.Delimiter()); return t.OStream(o).os; }
inline std::wostream& operator<<(std::wostream& os, const SQL_chinook_db_table_customers& t) { sqlite3pp::ostream_w o(os, t.Delimiter());  return t.OStream(o).os; }

#endif // !SQL_CHINOOK_DB_TABLE_CUSTOMERS_H
