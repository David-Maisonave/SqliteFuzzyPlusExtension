...
Example Usage:		(Using sqlite3pp::Table container)
	// Example #1
		...
		sqlite3pp::Table<SQL_TestData_db_table_Help> my_tbl;
		for (auto& row : my_tbl)
			std::cout << row << std::endl;

	// Example #2
		for (int i = 0; i < my_tbl.size(); ++i)
			std::cout << my_tbl[i].get_Function() << std::endl;

	// Example #3
		for (auto r = my_tbl.begin(); r != my_tbl.end(); ++r)
			std::cout << r->get_WikiAlias() << std::endl;
...
*/
#ifndef SQL_TESTDATA_DB_TABLE_HELP_H
#define SQL_TESTDATA_DB_TABLE_HELP_H
#include <string>
#include "..\sqlite3pp_EZ\sqlite3pp_ez.h"

class SQL_TestData_db_table_Help: public sqlite3pp::sql_base
{
...
public:
	...
	// Constructors
	SQL_TestData_db_table_Help() {} // Default constructor
	template <class T> SQL_TestData_db_table_Help(const T &t) { setData(t); } // Allows data input from different (or same) tables/views having the same data types and column names
	...
	{
		StrType strtype;
		strtype +=  "'" + sqlite3pp::to_string( Function) + "'";
		strtype +=  ",'" + sqlite3pp::to_string( Description) + "'";
		strtype +=  ",'" + sqlite3pp::to_string( Arguments) + "'";
		strtype +=  ",'" + sqlite3pp::to_string( Return) + "'";
		strtype +=  ",'" + sqlite3pp::to_string( Details) + "'";
		strtype +=  ",'" + sqlite3pp::to_string( ExtensionAlias) + "'";
		strtype +=  ",'" + sqlite3pp::to_string( WikiAlias) + "'";
		return strtype;
	}
	...
	Example Usage:		(Using sqlite3pp::TableOStream container)
			TableOStream<SQL_TestData_db_table_Help> tbl(DbFileNameArg("myDatabase.db"));
	...
	*/
	...
	friend std::ostream& operator<<(std::ostream& os, const SQL_TestData_db_table_Help& t);
	friend std::wostream& operator<<(std::wostream& os, const SQL_TestData_db_table_Help& t);
	static StrType Delimiter() { return "," ; }
	////////////////////////////////////////////////////////////////////////////////////////////
};
// sqlite3pp::TableOStream container interface.
inline std::ostream& operator<<(std::ostream& os, const SQL_TestData_db_table_Help& t) { sqlite3pp::ostream_a o(os, t.Delimiter()); return t.OStream(o).os; }
inline std::wostream& operator<<(std::wostream& os, const SQL_TestData_db_table_Help& t) { sqlite3pp::ostream_w o(os, t.Delimiter());  return t.OStream(o).os; }

#endif // !SQL_TESTDATA_DB_TABLE_HELP_H
