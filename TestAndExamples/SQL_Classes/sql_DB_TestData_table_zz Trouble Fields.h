...
Example Usage:		(Using sqlite3pp::Table container)
	// Example #1
		...
		sqlite3pp::Table<sql_DB_TestData_table_zz Trouble Fields> my_tbl;
		for (auto row : my_tbl)
			std::wcout << row << std::endl;

	// Example #2
		for (int i = 0; i < my_tbl.size(); ++i)
			std::wcout << my_tbl[i].get_Some__Names() << std::endl;

	// Example #3
		for (auto r = my_tbl.begin(); r != my_tbl.end(); ++r)
			std::wcout << r->get_Big__Integer__Support__3() << std::endl;
...
class sql_DB_TestData_table_zz__Trouble__Fields: public sqlite3pp::sql_base
{
...
protected:
	...
	UBigint Unsigned__Big__Integer__Support;
	UBigint Unsigned__Big__Integer__Support2;
	Bigint Big__Integer__Support;
	Bigint Big__Integer__Support__2;
	Bigint Big__Integer__Support__3;

public:
	using StrType = std::string;
	// Constructors
	sql_DB_TestData_table_zz__Trouble__Fields() {} // Default constructor
	template <class T> sql_DB_TestData_table_zz__Trouble__Fields(const T &t) { setData(t); } // Allows data input from different (or same) tables/views having the same data types and column names
	...
	Example Usage:		(Using sqlite3pp::TableOStream container)
			TableOStream<sql_DB_TestData_table_zz__Trouble__Fields> tbl(DbFileNameArg("myDatabase.db"));
	...
	*/
	...
	friend std::ostream& operator<<(std::ostream& os, const sql_DB_TestData_table_zz__Trouble__Fields& t);
	friend std::wostream& operator<<(std::wostream& os, const sql_DB_TestData_table_zz__Trouble__Fields& t);
	static StrType Delimiter() { return "," ; }
	////////////////////////////////////////////////////////////////////////////////////////////
};
// sqlite3pp::TableOStream container interface.
inline std::ostream& operator<<(std::ostream& os, const sql_DB_TestData_table_zz__Trouble__Fields& t) { sqlite3pp::ostream_a o(os, t.Delimiter()); return t.OStream(o).os; }
inline std::wostream& operator<<(std::wostream& os, const sql_DB_TestData_table_zz__Trouble__Fields& t) { sqlite3pp::ostream_w o(os, t.Delimiter());  return t.OStream(o).os; }

#endif // !SQL_DB_TESTDATA_TABLE_ZZ TROUBLE FIELDS_H
