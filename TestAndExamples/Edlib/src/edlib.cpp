...
#pragma warning( disable : 6262 6001 6011 6385 6308 )
// Note: Made changes to address most of the above warnings, but the compiler doesn't recognize the fixes as addressing these issues.
//       The correct way to fix these warnings is to refactor the source code, but that could make the implementation results different from the original edlib, and that is NOT the goal of including edlib in the SQLiteFuzzyPlusExtension.
//       The goal is to include the edlib library as-is, with the good, the bad, and the ugly.
            ...
            if (config.mode == EDLIB_MODE_HW) {  // If HW, I need to calculate start locations.
                ...
                for (int i = 0; i < result.numLocations; i++) {
                    ...
                    if (endLocation == -1) {
                        ...
                        int bestScoreSHW = 0, numPositionsSHW = 0;
...
static inline std::array<int, WORD_SIZE> getBlockCellValues(const Block block) {
    ...
    scores[static_cast<std::array<int, 64Ui64>::size_type>(WORD_SIZE) - 1] = score;
    return scores;
}
...
static int myersCalcEditDistanceSemiGlobal(
    ...
    *numPositions_ = 0;
    ...
    if (lastBlock == maxNumBlocks - 1) {
        ...
        for (int i = 0; i < W; i++) {
            int colScore = blockScores[static_cast<std::array<int, 64Ui64>::size_type>(i) + 1];
...
static int obtainAlignmentTraceback(const int queryLength, const int targetLength,
    ...
    *alignment = static_cast<unsigned char*>(malloc((static_cast<unsigned long long>(queryLength) + targetLength - 1) * sizeof(unsigned char)));
    *alignmentLength = 0;
...
static int obtainAlignment(
    ...
    if (alignmentDataSize < static_cast<long long>(1024) * 1024) {
...
static int obtainAlignmentHirschberg(
    ...
    // Unwrap the right half (I also reverse it while unwrapping).
    ...
    //--------------------- Find the best move ----------------//
    // Find the query/row index of cell in left column which together with its lower right neighbor
...
static string transformSequences(const char* const queryOriginal, const int queryLength,
    ...
    if (*queryTransformed == NULL || *targetTransformed == NULL)
        return "";
