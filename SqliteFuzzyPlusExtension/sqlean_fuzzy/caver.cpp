...
//#ifdef BUILD_WITH_FUZZY_CAVER
#include <assert.h>
#include <stdlib.h>
#include <string.h>
#pragma warning( disable : 6011 6387 4996 )
...
static char* remove_non_letters(const char* src) {
    ...
    char* res = (char*)malloc((src_len + 1) * sizeof(char));
    const char* src_it;
    char* res_it = res;
...
// replace_start replaces the `old` substring with the `new_str` one
// if it matches at the beginning of the `src` string
static char* replace_start(const char* src, const char* old, const char* new_str)
{
    size_t src_len = strlen(src);
...
// replace_end replaces the `old` substring with the `new_str` one
// if it matches at the end of the `src` string
static char* replace_end(const char* src, const char* old, const char* new_str) {
    size_t src_len = strlen(src);
    size_t old_len = strlen(old);
    size_t new_len = strlen(new_str);
    assert(new_len <= old_len);
    ...
    if (strncmp(src + src_len - old_len, old, old_len) == 0) {
        strncpy(res + src_len - old_len, new_str, new_len);
        *(res + src_len - old_len + new_len) = '\0';
    } else {
...
// replace replaces all `old` substrings with `new_str` ones
// in the the `src` string
static char* replace(const char* src, const char* old, const char* new_str) {
    size_t src_len = strlen(src);
    size_t old_len = strlen(old);
    size_t new_len = strlen(new_str);
    assert(new_len <= old_len);
    ...
    for (size_t idx = 0; idx < src_len;) {
        ...
        if (strncmp(src_it, old, old_len) == 0) {
            strncpy(res_it, new_str, new_len);
            res_it += new_len;
            idx += old_len;
        } else {
            *res_it = *src_it;
...
// with the `new_str` substring in the the `src` string
static char* replace_seq(const char* src, const char old, const char* new_str) {
    size_t src_len = strlen(src);
    size_t new_len = strlen(new_str);
    char* res = (char*)malloc((src_len + 1) * sizeof(char));
    const char* src_it;
