...
#include <string.h>
#include "common.h"
#pragma warning( disable : 6011 4267 )
...
unsigned levenshtein(const char* str1, const char* str2) {
    ...
    unsigned* vector = (unsigned*)calloc(str1_len + 1, sizeof(unsigned));
    for (col = 1; col <= str1_len; col++) {
        vector[col] = col;
    }

    // iterate through the imagined rows of arrays
    for (row = 1; row <= str2_len + 1; row++) {
        vector[0] = row;
        last_diag = row - 1;  // remember the last first slot

        // iterate through each member of the vector
        for (col = 1; col <= str1_len; col++) {
