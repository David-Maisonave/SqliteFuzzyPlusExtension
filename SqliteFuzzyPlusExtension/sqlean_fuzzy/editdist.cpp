...
** end of the word.
*/
constexpr auto FINAL_INS_COST_DIV = 4;
...
int edit_distance(const char* zA, const char* zB, int* pnMatch) {
    ...
    int nMatch = 0;
    int mStack[60 + 15] = { 0 }; /* Stack space to use if not too much is needed */
  ...
  printf("A=\"%s\" B=\"%s\" dc=%c\n", zA, zB, dc?dc:' ');
    ...
    if ((size_t)nB < (sizeof(mStack) * 4) / (sizeof(mStack[0]) * 5)) {
        ...
        m = toFree = (int*)malloc((static_cast<unsigned long long>(nB) + 1) * 5 * sizeof(m[0]) / 4);
        if (m == 0)
            return -3;
    }
    cx = (char*)&m[nB + 1];
