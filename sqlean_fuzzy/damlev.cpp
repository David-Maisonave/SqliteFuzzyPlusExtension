...
unsigned damerau_levenshtein(const char* str1, const char* str2) {
    ...
    if (str1_len == 0)
        return (unsigned)str2_len;
    if (str2_len == 0)
        return (unsigned)str1_len;
    ...
    const unsigned INFINITY = (unsigned)str1_len + (unsigned)str2_len;
    // create "dictionary"
    unsigned* dict = (unsigned*)calloc(alpha_size, sizeof(unsigned));
    size_t m_rows = str1_len + 2;  // matrix rows
    size_t m_cols = str2_len + 2;  // matrix cols
    // matrix to hold computed values
    unsigned** matrix = (unsigned**)malloc(m_rows * sizeof(unsigned*));
    for (unsigned i = 0; i < m_rows; i++)
        matrix[i] = (unsigned*)calloc(m_cols, sizeof(unsigned));
    ...
    // fill in the matrix
    for (unsigned row = 1; row <= str1_len; row++)
    {
        unsigned db = 0;
        for (unsigned col = 1; col <= str2_len; col++)
        {
            unsigned i = dict[(unsigned)str2[col - 1]];
            unsigned k = db;
            unsigned cost = EQ(str1[row - 1], str2[col - 1]) ? 0 : 1;
            if (cost == 0)
                db = col;
            matrix[row + 1][col + 1] =
